...
Chat = {
  ...
  sevenTvPaintToCss: function (paint) {
    ...
    gradients.forEach(function (gradient) {
      ...
      if (Array.isArray(gradient.canvas_size) && gradient.canvas_size.length) {
  ...
  shouldLoadSevenTvNamePaint: function (userId) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(Chat.info.seventvPaintCache, userId)
    ) {
      return false;
    }
    ...
    $.getJSON("https://7tv.io/v3/users/twitch/" + encodeURIComponent(userId))
      .done(function (connectionRes) {
        ...
        if (
          ...
          Chat.loadSevenTvUserStyleV4(sevenTvUserId).done(function (styleRes) {
  ...
  getKickBadgeType: function (badge) {
    ...
    if (rawType === "bot" || rawType === "kickbot" || rawType === "kick-bot") {
      return "bot";
    }
  ...
  createKickBadgeHtml: function (type, badge) {
    type =
      Chat.getKickBadgeType(type) || Chat.getKickBadgeType(badge) || "unknown";
